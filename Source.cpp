...
int blueHmin = 0, blueSmin = 78, blueVmin = 139;
int blueHmax = 24, blueSmax = 255, blueVmax = 255;
...
int main(void) {
	...
	while (vid.read(frame)) {
		...
		memcpy(frame_4.data, imageBlueIsolated->data, video.width* video.height * 3);
		...
		for (int segmentation = 0; segmentation < SIZE; segmentation++) {
			...
			if (blobOut != NULL) {
				...
				int num = *nLabels;
				segmentation==0? totalLabelsRed++: totalLabelsBlue++;
			}
		}
		...
		for (int pos = 0; pos < coordChannelVector.size(); pos++) {
				if (coordChannelVector[pos] != 0) {
					...
					int q1b = coord->q1b;
					int q1p = coord->q1p;
					cv::putText(frame_1,"Placa identificada -"+coordChannelColor[pos], cv::Point(coord->eixoXstart, coord->eixoYend + 15), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(170, 255, 25), 1.5);
				}
		}
